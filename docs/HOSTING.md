...
2026-09-24 线上冒烟用五日合成样本跑通单次买入持有、每日滚动（4 个窗口完成，末尾 1 个持有期不足而跳过）及开发区间/留出验证；页面显示进度完成并保存结果。这只验证静态部署与合成流程，不代表真实行情结果。浏览器版不是全部本机平台功能的迁移版。需要真实数据时，访客需从本机投资研究室导出快照包，再导入自己的浏览器；来源许可由分享者自行核验。真实 `.json.gz` 文件往返、完整交易规则一致性、跨浏览器兼容和不同设备性能仍需专项验收，不能据此宣称与本机平台完全等价。
