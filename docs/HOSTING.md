...
[GitHub Pages 使用限制](https://docs.github.com/en/pages/getting-started-with-github-pages/github-pages-limits)明确表示 Pages 不可作为在线业务或商业 SaaS 的免费主机。浏览器本地运行且没有云账户/云端用户数据的开源研究工具，是否符合预期使用范围应在实际上线前确认；确认前不把 Pages 当成已批准的多人在线 SaaS 托管方案。
...
GitHub Pages 可作为该服务的静态前端；真正的 API/工作队列/持久化存储必须部署在可运行后端的环境中。若坚持只用 GitHub 免费服务，可先做静态介绍或独立的浏览器端演示，但这不等于当前完整投资研究室已可供多人在线使用。部署前还必须做一次仓库历史和公开数据审查。
...
现在已有独立的静态浏览器版原型，源文件在 `pages/`，由 `scripts/build_pages_site.py` 按允许清单生成到忽略跟踪的 `build/pages/`。构建会拒绝白名单外的遗留文件和符号链接，避免把构建目录中的意外文件加入公开制品。GitHub Actions 工作流 `.github/workflows/pages.yml` 在 `main` 更新时构建并部署；仓库需先在 GitHub Pages 设置中选择 GitHub Actions。工作流使用最小化的 job 权限、固定到完整提交 SHA 的官方 Actions，并由 Dependabot 每周提出版本更新。它只打包显式列出的网页与 Python 核心文件，不复制本机应用数据目录。
...
它仍是原型，尚未公开部署，也不是全部本机平台功能的迁移版。需要真实数据时，访客需从本机投资研究室导出快照包，再导入自己的浏览器；来源许可由分享者自行核验。没有云账户、跨设备同步或服务器持久化。网页只运行固定内置策略，禁用任意 Python 策略上传/执行。浏览器版的滚动起点与留出流程、完整交易规则一致性、真实快照导入、跨浏览器兼容和性能仍需专项验证，不能据此宣称与本机平台完全等价。

固定内置策略在 Worker 中运行并不自动构成安全沙箱。本原型不接受任意 Python；增加自定义策略前必须先完成代码隔离和浏览器权限审查。研究数据、配置和结果保存在访问者浏览器的 IndexedDB/localStorage，丢失浏览器站点数据会一并删除；建议定期导出备份。Pyodide 固定版本由 jsDelivr 加载，CDN 可见常规网络请求信息，但请求不包含页面内快照或回测结果。尚未测首屏下载量、完整离线能力或 CDN 故障恢复；公开前应审计全部第三方依赖并测试主流浏览器和缓存策略。
...
文档或配置更新可以不传 `-Tests`；Python/JavaScript 代码改动必须传相关 pytest 文件或测试节点。`TASK_STATE.md`、`docs/changes/`、个人需求文档、个人 `config/universe.yaml`、个人策略/研究和验证证据保存在本机，不上传；只把通用用户文档与 `CHANGELOG.md` 放入公开版本。初始公开提交使用 `.public-history-baseline` 标记，发布脚本会检查标记和历史路径。旧的本机历史只保留在本机；配置空 GitHub 仓库作为远端后，才启用普通推送和 Pages。
